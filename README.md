...
| 실험 | w_normal | w_move | w_fall | NORMAL | MOVE | FALL |
|------|---------|--------|--------|--------|------|------|
| 자동 | 0.43 | 2.09 | 4.70 | 95.0% | 83.1% | 53.2% |
| 조정 | 0.50 | 2.00 | 10.0 | 99.1% | 79.4% | 34.9% |

- FALL 가중치를 높이면 오히려 성능 하락 — FALL 데이터 자체가 MOVE/NORMAL과 패턴 겹침
- 주요 오분류: FALL → MOVE 29개 (낙상 동작이 뒤척임과 유사)

**FALL 데이터 품질 분석**
- `--duration 2` 수집 시 구성: [정지 0.5초] + [낙상 0.5초] + [바닥 누운 상태 1초]
- 낙상 후 바닥 누운 상태는 의도적으로 포함 (낙상 패턴의 일부)
- 문제: 바닥 누운 상태가 소파 위 NORMAL 패턴과 CSI상 구분 어려움
- 해결 방향: FALL 데이터 추가 수집(2초, 횟수 증가)으로 낙상 패턴 다양성 확보
...
- [ ] FALL 데이터 추가 수집 (2초 × 30회 이상) 후 재학습
- [ ] 실환경 FALL 감지 테스트
